...
import {Grid, Typography, IconButton, Paper, Box, Button,
  Dialog, DialogTitle, DialogContent, DialogActions, TextField, RadioGroup, FormControlLabel, Radio
} from '@mui/material';
...
const ISRC_Member = ({selectedRole, setSelectedForm}) => {
  ...
  const fetchData = async () => {
    ...
    try{
      if(!fetchOnce.current) {
        ...
        const response = await axiosInstance.get('/api/investigator/projects', {params : 
          {type : "pbac_member",  role : selectedRole}});
          ...
          projectsData.map((item, index) => (
            <Paper key={index} elevation={2}>
              <Grid container spacing={2} alignItems="center" sx={{ p: 1.2, border: "0.5px solid #e3dddd" }}>
                ...
                <Grid item size={1}><Typography sx={commonTextStyle}>{item?.pbac_status || "pending"}</Typography></Grid>
