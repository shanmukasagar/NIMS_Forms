  ...
  const fetchData = async () => { //Get all projects
    ...
    try{
      if(!fetchOnce.current) {
        ...
        const response = await axiosInstance.get('/api/investigator/projects', {params : {type : "pbac_secretary"}});
    ...
    <Box>
      ...
      {Object.keys(projectsData).length > 0 ? projectsData?.projects.map((item, index) => (
        <Grid container spacing={2} alignItems="center" key={index} sx={{ p: 2, border: '0.5px solid #d1cdcd' }}>
          ...
          <Grid item size={2} sx = {{display : "flex", gap : "25px"}}>
          ...
          {item?.pbac_reviewer_name && item?.pbac_reviewer_name !== "" ? (
            <Grid item size = {3}>
              <TextField fullWidth value = {item?.pbac_reviewer_name} disabled />
            </Grid>
          ) : (
            <Grid item size={3}>
