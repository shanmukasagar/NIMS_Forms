...
export const getSubmittedData = async (tableName, setData ) => {
    try {
        let response;
        if(tableName === "") {
            response = await axiosInstance.get('/api/funded/formData', { params: { tableName } });
        }
        else if(tableName === "") {
            response = await axiosInstance.get('/api/selffunded/formData', { params: { tableName } });
        }
        else if(tableName === "") {
            response = await axiosInstance.get('/api/funded/formData', { params: { tableName } });
        }
        ...
        console.error("Error occurred while fetching report forms data:", error.message);
    }
};
